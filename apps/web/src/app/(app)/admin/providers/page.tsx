...
function OnboardInternalSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(
        `${res.data.data.category} staff onboarded and active. They'll need to log out and back in for the provider dashboard to unlock.`
      );
      setEmail('');
      setLicenseNumber('');
    } catch (err) {
...
function ProvidersContent() {
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <h1 className="font-display text-2xl font-bold text-navy">Providers</h1>
